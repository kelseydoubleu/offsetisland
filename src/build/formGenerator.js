...
export function fitCameraToBuilding() {
  ...
  // Include current building mesh group (check both buildingMeshGroup and buildingGroup)
  ...
  } else if (buildingGroup && buildingGroup.children.length > 0) {
    const currentBox = new THREE.Box3().setFromObject(buildingGroup);
    box.union(currentBox);
  }
...
export function addBuildingToPreview() {
  // Check both buildingMeshGroup and buildingGroup since generators may add to either
  let sourceGroup = null;
  if (buildingMeshGroup && buildingMeshGroup.children.length > 0) {
    sourceGroup = buildingMeshGroup;
  } else if (buildingGroup && buildingGroup.children.length > 0) {
    sourceGroup = buildingGroup;
  }
  ...
  // Move all children from source to previewGroup
...
export function commitCurrentBuilding() {
  ...
  // Check both buildingMeshGroup and buildingGroup since generators may add to either
  const hasUncommittedBuilding =
    (buildingMeshGroup && buildingMeshGroup.children.length > 0) ||
    (buildingGroup && buildingGroup.children.length > 0);

  if (hasUncommittedBuilding) {
...
export function getPreviewBuildingsWithGroups() {
  ...
  // Add all committed buildings (after commitCurrentBuilding is called)
  for (const building of committedBuildings) {
  ...
  // Add all preview buildings (before they're committed)
  for (const building of previewBuildings) {
    if (building.group && building.group.children.length > 0) {
      result.push({
        group: building.group,
  ...
  // Also include current uncommitted building mesh from buildingMeshGroup
  if (buildingMeshGroup && buildingMeshGroup.children.length > 0) {
    result.push({
...
export function getPreviewMeshesForWalk() {
  ...
  // Clone preview buildings (before they're committed)
  for (const building of previewBuildings) {
    if (building.group) {
      const cloned = building.group.clone(true);
      cloned.traverse(obj => {
  ...
  // Also clone current uncommitted building if it exists in buildingMeshGroup
  if (buildingMeshGroup && buildingMeshGroup.children.length > 0) {
...
export function renderPreviewToIsoImage(width = 200, height = 300) {
  ...
  // Collect all committed buildings
  for (const building of committedBuildings) {
    if (building.group) {
  ...
  // Collect all preview buildings (before they're committed)
  for (const building of previewBuildings) {
    if (building.group) {
      allGroups.push({ group: building.group, offset: building.offset });
    }
  ...
  // Also include current uncommitted building from buildingMeshGroup
  if (buildingMeshGroup && buildingMeshGroup.children.length > 0) {
