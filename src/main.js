...
function getTileSvgCoords(gx, gy) {
  ...
  // Use tilesByKey map for O(1) lookup instead of O(n) find()
  const tile = state.island.tilesByKey?.get(gx + ':' + gy);
  const isPlan = state.viewMode === 'plan' || state.viewMode === 'figureplan';
...
window.markWalkBuildingsStale = () => {
  // If walk mode is currently active, refresh buildings immediately
  if (WALK.active && WALK.group) {
    refreshWalkBuildings(state);
  } else {
    // Otherwise, mark stale for next activation
